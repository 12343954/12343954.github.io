...
def main():
    ...
    def aubo10_inv(dh_params, f):
        ...
        for i in range(8):
            ...
            if i < 4:
                theta_all[i, 0] = atan2(
                    C1, sqrt(A1 * A1 + B1 * B1 - C1 * C1)) - atan2(A1, B1)
            else:
                theta_all[i, 0] = atan2(
                    C1, -sqrt(A1 * A1 + B1 * B1 - C1 * C1)) - atan2(A1, B1)
            ...
            # theta_5
            A6 = (f[0, 1] * sin(theta_all[i, 0]) - f[1, 1] *
                  cos(theta_all[i, 0])) / sin(theta_all[i, 4])
            B6 = (f[1, 0] * cos(theta_all[i, 0]) - f[0, 0] *
                  sin(theta_all[i, 0])) / sin(theta_all[i, 4])
            theta_all[i, 5] = atan2(A6, B6)

            # theta_1 : 2 sets of solutions
            A234 = f[2, 2] / sin(theta_all[i, 4])
            B234 = (f[0, 2] * cos(theta_all[i, 0]) + f[1, 2] *
                    sin(theta_all[i, 0])) / sin(theta_all[i, 4])
            M = dh_params[4, 0] * A234 - dh_params[5, 0] * sin(theta_all[i, 4]) * B234 + f[0, 3] * cos(
                theta_all[i, 0]) + f[1, 3] * sin(theta_all[i, 0])
            N = -dh_params[4, 0] * B234 - dh_params[5, 0] * \
                sin(theta_all[i, 4]) * A234 - dh_params[0, 0] + f[2, 3]
            L = (M * M + N * N + dh_params[1, 1] * dh_params[1, 1] - dh_params[2, 1] * dh_params[2, 1]) / (
                2 * dh_params[1, 1])
            if i % 2 == 0:
                theta_all[i, 1] = atan2(
                    N, M) - atan2(L, sqrt(M * M + N * N - L * L))
            else:
                theta_all[i, 1] = atan2(
                    N, M) - atan2(L, -sqrt(M * M + N * N - L * L))
            ...
            # theta_2 and theta_3
            A23 = (-M - dh_params[1, 1] *
                   sin(theta_all[i, 1])) / dh_params[2, 1]
            B23 = (N - dh_params[1, 1] *
                   cos(theta_all[i, 1])) / dh_params[2, 1]
    ...
    frames = [Frame.from_euler_3(np.array([0.5 * pi, 0., pi]), np.array([[0.28127], [0.], [1.13182]])),
              Frame.from_euler_3(
                  np.array([0.25 * pi, 0., 0.75 * pi]), np.array([[0.48127], [0.], [1.13182]])),
