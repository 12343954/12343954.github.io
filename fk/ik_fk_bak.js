...
function inverse(x0, y0, z0) {
    ...
    if (status[0] == 0) {
        thetaA = status[1];
        status = delta_calcAngleYZ(x0 * cos120 + y0 * sin120, y0 * cos120 - x0 * sin120, z0, thetaB);  // rotate coords to +120 deg
    }
    if (status[0] == 0) {
        thetaB = status[1];
        status = delta_calcAngleYZ(x0 * cos120 - y0 * sin120, y0 * cos120 + x0 * sin120, z0, thetaC);  // rotate coords to -120 deg
    }
...
function test_bounds() {
    ...
    $('#bounds').html("X=" + roundoff(-sum, 3) + " to " + roundoff(sum, 3) + " mm"
        ...
        + "<br />Z=" + roundoff(z_middle - sum, 3) + " to " + roundoff(z_middle + sum, 3) + " mm");
